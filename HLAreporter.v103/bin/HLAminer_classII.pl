...
sub reportAlignments{
   ...
   ##my comments, self added. to label exemption contigs for not being excluded, first find out all alleles pointed by contig with 180 bp above and mark these alleles.
   my $exempt;
   foreach my $tig(sort {$cnt->{$a}{'support'}<=>$cnt->{$b}{'support'}  } keys %$cnt) {
      my $hitlist=$rec->{$tig};
      foreach my $hit (sort {$tnc->{$b}{'support'}<=>$tnc->{$a}{'support'}} keys %$tnc){
   ...
   foreach my $tig (sort {$cnt->{$a}{'support'}<=>$cnt->{$b}{'support'}  } keys %$cnt){### CYCLE THROUGH CONTIGS
      ...
      ##my comment, self added. if a contig points to multiple ag, set $covpoint = 0 so $score of allele supported by only this single contig will be 0 and not be recognized.
      ##find out the contigs pointing to multi genes and lable these contigs.
      my $ambitig = "FALSE";
      my $pre_ag = "NA";
      foreach my $hit (sort {$tnc->{$b}{'support'}<=>$tnc->{$a}{'support'}} keys %$tnc){
         if(defined $hitlist->{$hit}){
      ...
      ##if this is a multi-gene contig, see whether or not the alleles it points to are among the exempt allele list.
      my $validtest = "FALSE";
      if ($ambitig eq "TRUE") {
         foreach my $hit (sort {$tnc->{$b}{'support'}<=>$tnc->{$a}{'support'}} keys %$tnc){
            if(defined $hitlist->{$hit}){
               if ($exempt->{$hit} eq "TRUE") {
                  $validtest = "TRUE";
               }
            }
         }
            if ($validtest eq "FALSE") { ##this condition ensure those real short contigs are not eliminated from consideration
               $covpoint = 0;
            } 
      }
      ##my comments, above self added.
      ...
      foreach my $ag(keys %$tmpscore){
         ...
         foreach my $codvar(keys %$hlalist){
            ...
            if($hlalist->{$codvar}{'score'} == 0){ $uncountedtig->{$ag}{$codvar}{$tig} = "true"; $score->{$ag}{$codvar} += 1; } ##my comment, self added. mark this tig for giving mess an x letter later. score adds 1 is meant to mark the existance of this contig.
         }
      }
      $seen = 0;
      ####
   ...
   foreach my $ag(@hlagenes){
      ...
      foreach my $cv (sort {$list->{$b}<=>$list->{$a}} keys %$list){###sorted by decreasing score
         ...
         foreach my $tig(keys %$numtig){if($uncountedtig->{$ag}{$cv}{$tig} eq "true"){$mess.="x"; }} ##my comment, self added, which means this kind of tig does not count.
         ...
         if( $mess eq "" || length($mess) < $tt ){
            ...
            if ( ($score->{$ag}{$cv}) != 1 ) { ##my comments, we dont recognize those alleles supported by contigs pointing to multiple ag
