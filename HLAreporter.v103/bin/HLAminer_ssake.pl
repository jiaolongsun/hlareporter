...
sub reportAlignments{
   ...
   my $lengthinfofile = ""; ##my comments; self added
   my $readlength = 0; ##my comments; self added
   $lengthinfofile = `sed -n 1p patient.fof`;
   $readlength = `awk '{if(NR%4==2) {print length($1); exit 0;}}' $lengthinfofile`;
   ...
   foreach my $ag(@hlagenes){
      ...
      foreach my $cv (sort {$list->{$b}<=>$list->{$a}} keys %$list){###sorted by decreasing score
         ...
         if($mess eq "" || length($mess) < $tt || $tt >= 2 ){
			...
			#my $readlength = 0; ##my comments; self added
			if ( $tt >= 2 && $tt < 5 || ( $tt == 1 && $mess eq "" ) ) { ##my comments, if supported by >= 5 contigs, count this allele candidate
				##my comments, self added, this condition ensures at least one tig is greater than 2*readlength or 200 bp, this condition must occur before the "5 times $tiglowqualityflag standard"
				foreach my $tig(keys %$numtig){ $_=$tig; if(/size(\d+)/) { if($1 >= ($readlength * 2) || $1 >= 200 || ($1 >= 150 && $tt == 4)) { $tiglowqualityflag = 0; last; } else{$tiglowqualityflag = 1;} } }
			}
            if ( $tiglowqualityflag == 0 ) {
