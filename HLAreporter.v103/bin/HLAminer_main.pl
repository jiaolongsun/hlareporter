   ...
   foreach my $qry (keys %$top_hits){
      ...
      foreach my $si (sort {$b<=>$a} keys %$si_list){
         ...
         foreach my $hit (keys %$hitlist){
             ...
             #print "$qry,$hit\n" if ($reciprocal);
             if($reciprocal || (! $reciprocal && $d[10]>=$minid && $d[0]==100)){ ##my comments, now main step only allows [100%/100%]
...
sub reportAlignments{
   ...
   foreach my $ag(@hlagenes){
      ...
      foreach my $cv (sort {$list->{$b}<=>$list->{$a}} keys %$list){###sorted by decreasing score
		 ...
		 my $highqualityflag=0; ##my comments, self added, ensure that predictions with 2 high quality allele supprot wont be missed
		 foreach my $tig(keys %$numtig){ $_=$tig; if(/size(\d+)/) { if($1<170) {$highqualityflag=0; last; } else{$highqualityflag=1;} } }
		 if($highqualityflag==0){ foreach my $tig(keys %$numtig){ $_=$tig; if(/cov(\d+)/) { if($1<5) {$highqualityflag=0; last; } else{$highqualityflag=1;} } } } ##if all cov >= 5, also consider as highquality

         if( $mess eq "" || length($mess) < $tt || (length($mess) == $tt && $highqualityflag == 1 && ($tt > 1) ) || $tt > 2 ){

            my $chance = (1-$probability->{$ag}{$cv}) * 100;
            $probability->{$ag}{$cv} = 1 if(! $probability->{$ag}{$cv});
            my $chancescore = (log($probability->{$ag}{$cv})/log(10)) * -10;
			my $globalprevtmpscore = 0; ##my comments, self added
			my $tiglowqualityflag = 0; ##my comments, self added
			if ( $tt >= 4 ) {
				
				##my comments, self added, this condition ensures at least one tig is greater than 200, this condition must occur before the "5 times $tiglowqualityflag standard"
				foreach my $tig(keys %$numtig){ $_=$tig; if(/size(\d+)/) { if($1>150) { $tiglowqualityflag = 0; last; } else{$tiglowqualityflag = 1;} } if(/cov(\d+)/) { if($1>10) { $tiglowqualityflag = 0; last; } } }
			}

			if ( $tt == 3 ) {
				
				##my comments, self added, this condition ensures at least one tig is greater than 200, this condition must occur before the "5 times $tiglowqualityflag standard"
				foreach my $tig(keys %$numtig){ $_=$tig; if(/size(\d+)/) { if($1>160) { $tiglowqualityflag = 0; last; } else{$tiglowqualityflag = 1;} } }
			}
			...
			if ( $tt == 2 ) {
				...
				foreach my $tig(keys %$rec){ ##my comments, self added
					if ( ($globalprevtmpscore != 0) && ($globaltmpscore->{$ag}{$cv}{$tig} ne "") ) { ## my comments, self added, make sure to only use the non null value when making comparison
						if ( $globaltmpscore->{$ag}{$cv}{$tig} < ($globalprevtmpscore)/5 || $globaltmpscore->{$ag}{$cv}{$tig} > ($globalprevtmpscore) * 5 ) {
							$tiglowqualityflag = 1;
						}
					}
					if ($globaltmpscore->{$ag}{$cv}{$tig} ne "") {
						$globalprevtmpscore=$globaltmpscore->{$ag}{$cv}{$tig};
						#print "$ag,$cv,tig is $tig\n"; ##my comments, self added, for debug only
						#print "tmpscore is $globaltmpscore->{$ag}{$cv}{$tig}\n";
						#print "previous score is $globalprevtmpscore\n";
					}
				}
			}
			...
			if ( $tt == 1 ) {
				...
				##my comments, self added, this condition ensures tig is greater than 200 for situation with only one tig support
				foreach my $tig(keys %$numtig){ $_=$tig; if(/size(\d+)/) { if($1>200) { $tiglowqualityflag = 0; last; } else{$tiglowqualityflag = 1;} } }
			}

            if ( ($score->{$ag}{$cv}) > 1000 && $tiglowqualityflag == 0 ) {
