...
sub reportAlignments{
   ...
   #print Dumper($proc);
   my $globaltigprob;
   my $globaltmpscore; ##my comments, self added, this globaltmpscore can be seen at a level as $score could 
   my $normalizedscore; ##my comments, self added, this normalizedscore is the sum of all the subscores of according tigs without being used by the prediction with top score
      ...
      foreach my $hit (sort {$tnc->{$b}{'support'}<=>$tnc->{$a}{'support'}} keys %$tnc){### CYCLE THROUGH HLA
         if(defined $hitlist->{$hit}){
            ...
            if($notnullallele){##added 20dec2011
			   ...
			   $globaltmpscore->{$ag}{$hit}{$tig} = ($sizepoint * $covpoint * $sidpoint * $rbhpoint); ##my comments, self added, make a copy to $globaltmpscore
   ...
   ### TABLE SUMMARY
   my $bt; ##my comments, $bt is located out of function @hlagenes, which means if a tig has been used by a gene, it is also viewed as used to other genes
   my $pdesignprob;
   my $trackgroupprob;
   my $refgene; ##my comments, used to record the reference hla gene in order not to report other noise genes after removing unique strategy length($mess) < $tt, like noise DRB3 to DRB1
   print LOG "\n";
   print LOG "-" x 70, "\n";
   ...
   foreach my $ag(@hlagenes){
      ...
      my $prevsupertype = "NA";
      my $prevmess = "";
	  my $maxscore = "-10"; ##my comments, this score record the top score value
	  my $maxscoreflag = 1; ##my comments, update the manscore value only when flag is 1
	  my $maxscoretigflag; ##my comments, record tigs which support maxscore case
	  my $maxnormalizedscore = "-10"; ##my comments, this score record the top normalizedscore value
      ...
      foreach my $cv (sort {$list->{$b}<=>$list->{$a}} keys %$list){###sorted by decreasing score
		 ...
		 if ($maxscoreflag == 1) { ##my comments, record the top score only when flag is 1
			 $maxscore = $list->{$cv};
			 $maxscoreflag = 0;
			 foreach my $tig(keys %$numtig) { $maxscoretigflag->{$tig} = 1; }##my comments, mark this tig as a tig that supports maxscore case
		 }

		 ####my comments, self added, compute normalizedscore
		 foreach my $tig(keys %$numtig) {
		 		 if( $maxscoretigflag->{$tig} != 1 ) {
		 			 $normalizedscore->{$ag}{$cv} += $globaltmpscore->{$ag}{$cv}{$tig};
		 		 }
		 }

         #if( $mess eq "" || length($mess) < $tt ){ ##my comments, original code
         if( ($mess eq "" || length($mess) < $tt) && (($score->{$ag}{$cv}) == $maxscore) ){ ##my comments, self added, first condiction guarantees it is not noise gene, second condiction guarantees only one allele with highest score in two is shown in this round

            my $chance = (1-$probability->{$ag}{$cv}) * 100;
            $probability->{$ag}{$cv} = 1 if(! $probability->{$ag}{$cv});
            my $chancescore = (log($probability->{$ag}{$cv})/log(10)) * -10;
            if ( ($score->{$ag}{$cv}) > ($maxscore)/5 ) { ##my comments, the threshold is set to be 5 times
				$refgene = $ag; ##my comments, used to record the reference hla gene in order not to report other noise genes after removing unique strategy length($mess) < $tt, like noise DRB3 to DRB1
				printf LOG "\t$ag,$supertype,$cv,$score->{$ag}{$cv},$normalizedscore->{$ag}{$cv},$tt,$mess,%.2e,%.1f\n", ($probability->{$ag}{$cv},$chancescore );
				#printf LOG "\t$ag,$supertype,$cv,$score->{$ag}{$cv},$tt,$mess,%.4f,%.2e,%.1f\n", ($chance,$probability->{$ag}{$cv},$chancescore );
			} else {
				printf LOG "\t$cv,$score->{$ag}{$cv},$tt,$mess,%.4f,%.2e,%.1f\n", ($chance,$probability->{$ag}{$cv},$chancescore );
			}
		 ...
		 else { ##my comments, self added, if this case is not the top score case, then compare its normalized score with max normalized score and update the max record
			 if ( ($normalizedscore->{$ag}{$cv}) > $maxnormalizedscore ) {
				 $maxnormalizedscore = ($normalizedscore->{$ag}{$cv});
			 }
		 }
	  ...
	  #my comments, self added, output predictions with highest normalizedscore
	  my $submaxscore = "-10"; ##my comments, this score record the top score value for cases with max normalizedscore
	  my $list = $normalizedscore->{$ag};
	  foreach my $cv (sort {$list->{$b}<=>$list->{$a}} keys %$list){###sorted by decreasing normalizedscore, and calculate max score of cases with max normalizescore
		  if ( ($normalizedscore->{$ag}{$cv}) == $maxnormalizedscore ) {
			  if( ($score->{$ag}{$cv}) > $submaxscore ) { $submaxscore = ($score->{$ag}{$cv}); }
		  }
	  }

	  foreach my $cv (sort {$list->{$b}<=>$list->{$a}} keys %$list){###sorted by decreasing normalizedscore
		  my $supertype = $1 if($cv=~/^([^\:]*)/);
		  my $numtig = $tracktig->{$ag}{$cv};
		  my $tt = keys (%$numtig);

		  if ( ($normalizedscore->{$ag}{$cv}) == $maxnormalizedscore  && ($ag eq $refgene) && ( ($score->{$ag}{$cv}) == $submaxscore ) ) {
			  my $chance = (1-$probability->{$ag}{$cv}) * 100;
			  $probability->{$ag}{$cv} = 1 if(! $probability->{$ag}{$cv});
			  my $chancescore = (log($probability->{$ag}{$cv})/log(10)) * -10;
			  printf LOG "\t$ag,$supertype,$cv,$score->{$ag}{$cv},$normalizedscore->{$ag}{$cv},$tt,mess,%.2e,%.1f\n", ($probability->{$ag}{$cv},$chancescore );
	  ...
	  #my comments, above are self added

   }


   #my comments, self added, determine real types through normalizedscore
   #foreach my $ag(@hlagenes){
	#   my $list = $normalizedscore->{$ag};
	#   foreach my $cv (sort {$list->{$b}<=>$list->{$a}} keys %$list){###sorted by decreasing normalizedscore
	#	   printf LOG "\t$cv\n";
