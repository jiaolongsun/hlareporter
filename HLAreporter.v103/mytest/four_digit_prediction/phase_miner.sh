...
if [[ $gapnumber > 1 ]]; then
./multigap_miner.sh ${string[1]} ${string[2]}
exit
fi
...
gapsize=`expr \`cat gapinfo.dat | cut -d ' ' -f 2\` - \`cat gapinfo.dat | cut -d ' ' -f 1\`` 
echo -e "\nNon-polymorphic gap (potential intronic gap not shown):\n$gapsize bp" >> report.out
echo -e "Gap location:\n`sed -n 1p gapinfo.dat`" >> report.out
